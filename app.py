...
def run_style_generation(style_name, visual_instruction, image_obj):
    try:
        ...
        # Fallback endpoints in case of API routing issues
        endpoints = [
        ...
        if not hf_res or hf_res.status_code != 200:
            status = hf_res.status_code if hf_res else "Timeout"
            return None, f"Visual engine busy (Status {status}). Please wait a minute and retry."
